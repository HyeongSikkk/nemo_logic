class NemoMaster :
    def __call__(self, row_lines, col_lines) :
        # 네모로직 보드 생성
        self.board = [[0 for _c in range(len(col_lines))] for _r in range(len(row_lines))]
        
        # 각 줄에 해당하는 경우의 수 발생
        row_casts, col_casts = self.make_casts(row_lines, col_lines)
        
        # 각 줄에 해당하는 경우의 수를 줄이기
        row_casts, col_casts = self.reduct_cast(row_casts, col_casts)
        
        # 풀어내기
        return self.solve(row_casts, col_casts)
    
    def make_casts(self, row_lines, col_lines) :
        rows_cnt = len(row_lines)
        cols_cnt = len(col_lines)
        row_casts = {}
        col_casts = {}
        calced = {}
        ...
        return row_casts, col_casts
    
    
    def is_can_cast(self, axis, key, cast) :
        # row
        if axis :
            for idx, ea in enumerate(cast) :
                board_ea = self.board[key][idx]
                if board_ea == 1 :
                    if ea == 0 :
                        return False            
                elif board_ea == -1 :
    ...
    def to_transport_board(self, tmp_borad, check) :
        if check :
    ...
    def reduct_cast(self, row_casts, col_casts) :
        rows_cnt, cols_cnt = len(row_casts), len(col_casts)
        while True :
            # 전부 채워져 있는 보드 생성
            ...
            self.to_transport_board(all_check_row, True)
            self.to_transport_board(all_check_col, True)
            self.to_transport_board(all_block_row, False)
            self.to_transport_board(all_block_col, False)
            ...
            for key in row_casts :
                ...
                for cast in row_casts[key] :
                    if self.is_can_cast(True, key, cast) :
            ...
            for key in col_casts :
                ...
                for cast in col_casts[key] :
                    if self.is_can_cast(False, key, cast) :
                        new_list.append(cast)
    ...
    def solve(self, row_casts, col_casts) :
    ...
    def print_board(self, simple = True) :
        if simple :
            for idx, t in enumerate(self.board) :
                if idx % 5 == 0 :
                    print()
